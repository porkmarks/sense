...
bool Item16::pack(const Data& last_data, const Data& data)
{
    ...
    m_humidity = static_cast<uint8_t>(std::clamp(ih, int16_t(0), int16_t(255)));

    float dt = data.temperature - last_data.temperature;
    int16_t idt = static_cast<int16_t>((dt + MAX_TEMPERATURE_DELTA) * TEMPERATURE_SCALE + 0.5f);
    m_temperature_delta = static_cast<uint8_t>(std::clamp(idt, int16_t(0), int16_t(127)));
    return true;
}
...
{
    int16_t ih = static_cast<int16_t>(data.humidity * GROUP_HUMIDITY_SCALE + 0.5f);
    m_humidity = static_cast<uint8_t>(std::clamp(ih, int16_t(0), int16_t(255)));

    int32_t it = static_cast<int32_t>(data.temperature * GROUP_TEMPERATURE_SCALE + 0.5f);
    m_temperature = static_cast<int16_t>(std::clamp(it, int32_t(-32767), int32_t(32767)));
}
...
bool Storage::Group::unpack_next(Storage::iterator& it) const
{
    ...
    if (uint16_t(it.offset) >= m_header.data_size)
    {
        return false;
    }
...
bool Storage::push_back(const Data& data)
{
    ...
    {
        Item8* item8 = (Item8*)(g.m_data + g.m_header.data_size);
        if (g.m_header.data_size + sizeof(Item8) <= Group::DATA_SIZE && item8->pack(m_last_data, data))
        {
        ...
        {
            Item16* item16 = (Item16*)(g.m_data + g.m_header.data_size);
