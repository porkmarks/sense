...
public:
    ...
    static constexpr uint16_t SLAVE_ADDRESS_BEGIN = BROADCAST_ADDRESS + 1;
    static constexpr uint16_t SLAVE_ADDRESS_END = SLAVE_ADDRESS_BEGIN + 14000;
...
#   pragma pack(pop)
    ...
    static const uint8_t MAX_USER_DATA_SIZE = RFM22B::MAX_PACKET_SIZE - sizeof(Header);

    uint8_t m_buffer[RFM22B::MAX_PACKET_SIZE];
    uint8_t m_offset = 0;
};
